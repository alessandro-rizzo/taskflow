# Taskflow architecture-bootstrap prototype

> [!IMPORTANT]
> This is the isolated original implementation. It remains buildable evidence,
> but new Taskflow product code is being designed from a clean foundation. See
> the root [product specification](../../docs/product-specification.md) and
> [risk-first roadmap](../../docs/roadmap.md).

The [prototype baseline](docs/baseline.md) records what this implementation
demonstrates, its important limitations, and the command used to reproduce it.
...
See [Architecture](docs/architecture.md) and the historical
[Roadmap](docs/roadmap.md) for the prototype's boundaries and implementation
order. The root [Product specification](../../docs/product-specification.md)
describes the longer-term typed, reproducible, native-target, and agent-first
direction.
...
This prototype is not production-ready and is not the active implementation
foundation. Its SSH provider remains a contract spike. Changes should preserve
its role as reproducible evidence rather than continue its historical roadmap.
