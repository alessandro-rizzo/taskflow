...
- Read `docs/product-specification.md`, `docs/roadmap.md`, and the selected
  Backlog ticket before proposing or changing implementation.
- Treat `docs/` as product truth, `experiments/` as disposable evidence, and
  `prototype/bootstrap/` as preserved evidence. New production code must not
  import the prototype module.
- Do not create a new production Go module or stabilise public packages before
  Gate 1 accepts the semantic model and trust boundaries.
- Use the Backlog CLI for every task mutation. Before work, run
  `backlog task view <id> --plain`, verify dependencies, then record the agent
...
- Work on one ticket per agent session. Parallel agents may take only tasks
  ...
  editing.
- Implement every ticket on a dedicated branch in an isolated Git worktree
  created from current `main`; never implement ticket work directly in the
  primary checkout. Record the branch, worktree path, and initially claimed
  files in the ticket notes.
- Keep one ticket per branch and worktree. Parallel agents must not share a
  worktree, branch, build output directory, or mutable fixture state.
- After the ticket is accepted and landing is explicitly authorised, update it
  against current `main`, rerun its checks, and merge it to `main`. Serialize
  merges through one integrator so parallel branches cannot race; verify
  `HEAD == origin/main` after pushing before removing the worktree.
- Tickets labelled `coordination-only` are milestone containers, not agent
  implementation work. Close them only after every child and gate ticket is
  complete.
- Coordinate shared contracts through the owning ticket. Do not independently
  invent schemas, fixture formats, or shared helpers from parallel tickets.
- Every experiment must retain its roadmap ID, predeclared thresholds, raw
  evidence location, limitations, recommendation, and one verification command.
- Keep task status, acceptance criteria, notes, and final summary current. A
  merged change is not a completed gate without the required evidence and gate
  decision.
- Validate from the repository root with `mise exec -- task check` and
  task-specific checks. Do not nest `mise exec` when delegating to the isolated
  prototype.
- Do not commit, merge, push, publish, or perform destructive cleanup without
